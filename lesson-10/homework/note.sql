...
SELECT EMP1.Name AS EmplyeeName, emp2.Name AS ManagerName FROM Employees AS EMP1
...
SELECT Departments.DepartmentName, COUNT(EmployeeID) AS EmployeeCount FROM Employees
INNER JOIN Departments
ON Employees.DepartmentID = Departments.DepartmentID
...
SELECT Customers.FirstName, Customers.LastName, COUNT(OrderID) AS TotalOrders FROM Customers
INNER JOIN Orders
ON Customers.CustomerID = Orders.CustomerID
...
ON EMP1.ManagerID = EMP2.ManagerID
...
SELECT EMP1.Name AS Employee1, EMP2.Name AS Employee2, EMP1.ManagerID, EMP1.Salary FROM Employees AS EMP1
INNER JOIN Employees AS EMP2
...
LEFT JOIN Courses
ON Enrollments.CourseID = Courses.CourseID
WHERE Students.StudentID NOT IN (SELECT Enrollments.StudentID FROM Enrollments 
