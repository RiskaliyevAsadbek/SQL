...
--                                  EASY TASKS
--TASK 1 You need to write a query that outputs "100-Steven King", meaning emp_id + first_name + last_name in that format using employees table.
SELECT CONCAT(EMPLOYEE_ID, '-', FIRST_NAME, ' ' ,LAST_NAME) FROM Employees
--TASK 2 Update the portion of the phone_number in the employees table, within the phone number the substring '124' will be replaced by '999'
SELECT REPLACE(PHONE_NUMBER, 124, 999) FROM Employees
--TASK 3 That displays the first name and the length of the first name for all employees whose name starts with the letters 'A', 'J' or 'M'. Give each column an appropriate label.Sort the results by the employees' first names.(Employees)
...
--TASK 7 You have to sort data based on the Id but Id with 0 should always be the last row. Now the question is can you do that with a single order by column.(SingleOrder)
SELECT * FROM SingleOrder
ORDER BY Id DESC
--TASK 8 Write an SQL query to select the first non-null value from a set of columns. If the first column is null, move to the next, and so on. If all columns are null, return null.(person)
...
SUBSTRING(FullName, CHARINDEX(' ', fullname,charindex(' ', fullname)+1) +1, len(fullname)) as lastname from Students
--TASK 2 For every customer that had a delivery to California, provide a result set of the customer orders that were delivered to Texas. (Orders Table)
SELECT * FROM Orders
...
select EMPLOYEE_ID, FIRST_NAME, LAST_NAME ,len(CONCAT(FIRST_NAME, LAST_NAME))-len(REPLACE(CONCAT(FIRST_NAME, LAST_NAME),'a', '')) as number_of_a from Employees
where len(CONCAT(FIRST_NAME, LAST_NAME))-len(REPLACE(CONCAT(FIRST_NAME, LAST_NAME),'a', '')) >=3
--TASK 5 The total number of employees in each department and the percentage of those employees who have been with the company for more than 3 years(Employees)
select DEPARTMENT_ID, count(EMPLOYEE_ID) as total_employee, sum(case when left(cast(HIRE_DATE as varchar), 4) <= CAST(YEAR(GETDATE())-3 as varchar)then 1 else 0 end) * 100 / count(EMPLOYEE_ID)  as percaentageover  from Employees
...
-- Most and least experienced Spacemen per job
SELECT 
  p1.JobDescription,
...
CREATE FUNCTION dbo.SplitAndClassifyChars(@input VARCHAR(MAX))
RETURNS TABLE
AS
RETURN
(
    ...
    SELECT
        (SELECT STRING_AGG(ch, '') FROM Chars WHERE ASCII(ch) BETWEEN 65 AND 90) AS UppercaseLetters,
        (SELECT STRING_AGG(ch, '') FROM Chars WHERE ASCII(ch) BETWEEN 97 AND 122) AS LowercaseLetters,
        (SELECT STRING_AGG(ch, '') FROM Chars WHERE ASCII(ch) BETWEEN 48 AND 57) AS Numbers,
        (SELECT STRING_AGG(ch, '') FROM Chars WHERE 
            ASCII(ch) NOT BETWEEN 48 AND 57 AND 
            ASCII(ch) NOT BETWEEN 65 AND 90 AND 
            ASCII(ch) NOT BETWEEN 97 AND 122) AS OtherChars
);
SELECT * FROM dbo.SplitAndClassifyChars('tf56sd#%OqH');

DECLARE @str VARCHAR(100) = 'tf56sd#%OqH';

SELECT
  STRING_AGG(CASE 
                WHEN ASCII(SUBSTRING(@str, number, 1)) BETWEEN 65 AND 90 THEN SUBSTRING(@str, number, 1) 
             END, '') AS UppercaseLetters,
  STRING_AGG(CASE 
                WHEN ASCII(SUBSTRING(@str, number, 1)) BETWEEN 97 AND 122 THEN SUBSTRING(@str, number, 1) 
             END, '') AS LowercaseLetters,
  STRING_AGG(CASE 
                WHEN ASCII(SUBSTRING(@str, number, 1)) BETWEEN 48 AND 57 THEN SUBSTRING(@str, number, 1) 
             END, '') AS Digits,
  STRING_AGG(CASE 
                WHEN ASCII(SUBSTRING(@str, number, 1)) NOT BETWEEN 48 AND 57
                     AND ASCII(SUBSTRING(@str, number, 1)) NOT BETWEEN 65 AND 90
                     AND ASCII(SUBSTRING(@str, number, 1)) NOT BETWEEN 97 AND 122 
                THEN SUBSTRING(@str, number, 1) 
             END, '') AS OtherChars
...
select s1.StudentID, s1.FullName, SUM(s2.Grade) as running_out from Students as s1
inner join Students as s2
on s1.StudentID >= s2.StudentID
group by s1.StudentID, s1.FullName
...
SET TotalSum =
  CASE
    WHEN Equation LIKE '%+%' AND Equation NOT LIKE '%+%+%' AND Equation NOT LIKE '%+%-%' THEN
    ...
    ELSE NULL  -- for multi-op or unsupported formats
  END;
...
where SUBSTRING(cast(s1.Birthday as varchar), 5, 6 ) = SUBSTRING(cast(s2.Birthday as varchar), 5, 6)
...
select GREATEST(PlayerA, PlayerB) as greatest_, LEAST(PlayerA, PlayerB) as least_, SUM(Score) as total_score from PlayerScores
