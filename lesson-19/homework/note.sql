...
('HR', 8);
go
create proc usp_EmployeeBonus
as
begin
select Employees.EmployeeID, Employees.FirstName + Employees.LastName as FullName, Employees.Department,
Salary, Employees.Salary * (DepartmentBonus.BonusPercentage/100) as BonusAmount into #EmployeeBonus  from Employees
join DepartmentBonus
...
go
alter proc usp_updated_employee_salary
@department varchar(100),
@bonuspercentage int
as
begin
update Employees
set Salary = (Salary * @bonuspercentage/100) + Salary
...
exec usp_updated_employee_salary 'Sales', 10
...
else 'Leaf' end as Tree_types from Tree
...
CREATE TABLE Confirmations (user_id INT, time_stamp DATETIME, action varchar(50) CHECK (action IN ('confirmed', 'timeout')))
...
go
with status as
(select Signups.user_id, count(case when Confirmations.action  = 'confirmed' then 1 else 0 end) as status, SUM(case when Confirmations.action  = 'confirmed' then 1 else 0 end) as summed from Signups
left join Confirmations
on Signups.user_id = Confirmations.user_id
group by Signups.user_id
)
select user_id, cast(summed as float) / cast(status as float) as confirmation_rate from status

/*📄 Task 6:
Find employees with the lowest salary.Find all employees who have the lowest salary using subqueries.*/
...
where salary = (select MIN(salary) from employees_)
...
as
begin
select Products.ProductName, SUM(Sales.Quantity) as total_quantity,
SUM(Products.Price * Sales.Quantity) as total_sales_amount, MIN(SaleDate) as first_sale_date,
...
exec usp_GetProductSalesSummary 9
