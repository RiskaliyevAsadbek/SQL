...
with cte as (
select *, 
DENSE_RANK() over (partition by customerID order by saleamount desc) as ranked 
from ProductSales
)
select * from cte
where ranked = 1
...
select ProductName, SaleDate, SaleAmount, previous_sale ,SaleAmount - previous_sale as difference from cte
...
select *, (next_sale / SaleAmount) * 100 - 100 as difference from cte

/* 9.Write a query to calculate the ratio of the current sale amount to the previous sale amount within the same product. */
go
with cte as(
select *, 
LAG(SaleAmount) over (partition by productname order by saledate) as previous_sal_amount
from ProductSales)

select *,  SaleAmount / previous_sal_amount as ratio_of from cte
...
from ProductSales)
select *, SaleAmount - first_sale_amount as difference from cte
...
/* 12. Write a query to calculate a "closing balance" (running total) 
   for sales amounts which adds the current sale amount 
   to a running total of previous sales. */
...
select *,
SUM(Salary) over (partition by department order by salary, EmployeeID  
...
--version 1 
select *,
SUM(Salary) over ( ORDER BY HireDate desc ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS SUM_SALARYlast3
from Employees1

--version 2
SELECT SUM(Salary) AS Sum_Last3Hires
FROM (
    SELECT TOP 3 Salary
    FROM Employees1
