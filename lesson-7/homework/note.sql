...
select max(Salary) as max_price from Employees
...
avg(SaleAmount) as average_sales from Sales
group by ProductID
--TASK 12 Write a query to count the number of employees from the Department HR.
select count(EmployeeID) as cnt_employee_hr from Employees
...
select DepartmentName, avg(Salary) as average_salary, count(EmployeeID) as number_of_employees from Employees
group by DepartmentName
...
select CustomerID, count(OrderID) as cnt_order from Orders
group by CustomerID
...
avg(Salary) average_salary  from Employees
group by DepartmentName
...
select custid, sum(case when freight > 50 then freight else 0 end) as total_amount_over50, 
sum(case when freight < 50 then freight else 0 end) as total_amount_lower50
from [Sales].[Orders]
group by custid
...
select OrderDate, sum(TotalAmount) as total_amount, count(distinct ProductID) number_of_product from Orders
group by OrderDate
...
max(TotalAmount) as max_amount from Orders
