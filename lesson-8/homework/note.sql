...
SELECT Category, COUNT(*) AS Total_Number_of_products FROM Products
GROUP BY Category
...
SELECT Country, COUNT(CustomerID) AS Total_number_of_customer FROM Customers
GROUP BY Country
...
WHERE OrderDate LIKE '2023-01-[0-9][0-9]' 
AND CustomerID NOT IN (SELECT CustomerID FROM Invoices
WHERE InvoiceDate LIKE '2023-01-[0-9][0-9]')

SELECT * FROM Orders
JOIN Invoices
ON Orders.CustomerID=Invoices.CustomerID
WHERE Orders.OrderDate LIKE '2023-01-[0-9][0-9]' AND Invoices.InvoiceDate NOT LIKE '2023-01-[0-9][0-9]'
...
SELECT OrderDate, AVG(TotalAmount) AS Average_order_amount FROM Orders
GROUP BY OrderDate
...
PIVOT
( SUM(Population) for district_name in ([Chilonzor], [Yakkasaroy], [Mirobod], [Bektemir])) AS PIVOT_TABLE
--                                 HARD-LEVEL TASKS
--TASK 19 Using Invoices table, show top 3 customers with the highest total invoice amount. Return CustomerID and Totalspent.
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS TOTAL_AMOUNT FROM Invoices
GROUP BY CustomerID
...
SELECT ProductName, COUNT(SaleID) AS CNT_PRODUCT FROM Products
JOIN Sales 
...
SELECT 
    district_name,
    Population
INTO City_Population_Recreated1
FROM Population_Each_City
UNPIVOT (
    Population FOR district_name IN ([Bektemir], [Chilonzor], [Yakkasaroy], [Mirobod])
) AS Unpivoted;
